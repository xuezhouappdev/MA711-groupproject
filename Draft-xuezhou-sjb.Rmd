...
The data_clean will be the cleaned dataset we used in the further variable summaries and comparisons.For numeric variabls, we calculate the basic summary statistics such as median, min, max, mode and quantile. We also use visualizations such as hisgroms and boxplots to understand the distribution of the values. For categorical variables, we use frequency table to understand the frequcies of each categories. Box chart is used as a visualization tool.
...
To compare the percentages of Pell Grant in different control type, we grouped the Percentage of Pell Grant data into the college type level to take a further analysis. 
...
As a result of all the statistical analysis, we conclude that the attributes such as percentage of students who receive Pell Grant, median earning six years from entry, amount of debt, default rate for student loan and predominant degree type will affect the students' post graudate earnings.
