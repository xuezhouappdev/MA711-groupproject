...
below is the code to generate stacked hisogram
``` {r}
data_clean %>%
  group_by(variable$STABBR, na.rm=TRUE) %>%
  ggvis(x=~NPT4_COMBINE,na.rm=TRUE) %>%
  layer_histograms(width=5, stack=TRUE, fill=~variable$STABBR,na.rm=TRUE)


data_clean %>%
  ggplot(aes(x=NPT4_COMBINE)) +
  geom_bar(binwidth=50,aes(fill=variable$ctrF,na.rm=TRUE))

data_clean %>%
  ggplot(aes(x=NPT4_COMBINE,na.rm=TRUE)) +
  geom_bar(binwidth=3000,aes(fill=variable$ctrF,na.rm=TRUE),breaks=seq(-20000, 90000, by =3000))
...
data_clean %>%
  ggplot(aes(x=NPT4_COMBINE,na.rm=TRUE)) +
  geom_bar(binwidth=50,aes(fill=variable$ctrF,na.rm=TRUE),breaks=seq(-20000, 90000, by =50))
