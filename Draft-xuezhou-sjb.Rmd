 ...
 * 5.1 Individual variable summary
  ...
  * 5.1.5 Earning
    * 5.1.5.1 Median family income in real 2015 dollars
    * 5.1.5.2 Earning after graduation
  * 5.1.6 Repayment
    * 5.1.6.1 3-year Repayment rate
    * 5.1.6.2 3-year Default rate
* 6.Relationship between variables
 * 6.1 NPT4_COMBINE VS State
 * 6.2 Debt VS Cost
 * 6.3 Percentage of Pell Grant VS college type(CONTROL)
 * 6.4 Percentage first-generation students VS Statu
 * 6.5 family income with Pell Grant
 * 6.6 cost and median debts with earning
 * 6.7 default with earning
 * 6.8 degree type vs. earning
 * 6.9 predominant degree, control  and earning after 6 year of entry
 * 6.10 predominant degree + control (count)
 * 6.11 control + earning
* 7. Comparison between variables
...
| Variable Name    | Data Type         | Description                                                          |    Category in Documentation       |
| --------------  | ------------------ | ---------------------------------------------------------------------| --------------------------------|
...
| variable$qreF    | Factor            |Predominant degree offered                                            |School       |
| variable$ctrF    | Factor            |Control of institution                                                |School     |
| variable$STABBR  | Factor            |State                                                                 |School     |
...
data_clean %>%
  ...
  geom_bar(binwidth=0.1,aes(fill=variable$ctrF ))
...
```
We use summary statistics, standard deviation and histogram to understand the distribution of this variable.

The summary statistics show that students in 50% of institution have a median debt of more than 5500, which is not very unaffordable. The median debt for attending students can be as much as 27620 for some school and as low as 600 for others, which indicates it may have a large variance. So we also calculate standard deviation to measure the dispersion of the distribution. The standard deviation is 2415.623 about 95% of institution have a median debt ranging from 1167.754  to 10830.246. The histogram shows that the distribution of the median roughly follows a standard distribution with a sharp peak and slightly heavy tail.
...
To analyze the relationship between average net cost for Title IV institutions and State, we grouped the data into the state level to take a further look.
```{R message=FALSE, warinig = FALSE}
...
##6.4  Percentage first-generation students VS Status
To compare the Percentage first-generation students with the different states, we grouped data into the state level to take a further analysis.
...
```
From the scatter plot, it is obvious that the MD_FAMINC is in a negative linear relationship with PCTPELL: the data show a downhill pattern from left to right; As the MD_FAMINC increase, the value of PCTPELL decreases. 
...
#7. Comparison between variables
 #7.1 PCTPELL VS PCTFLOAN

```{r}

library(ggplot2)
ggplot(data = data_clean) +
  geom_histogram(breaks=seq(0, 1, by =0.05),aes(x = PCTPELL, y=(..count..)), 
                       alpha=0.9, fill ="red",position="dodge") +
  geom_histogram(breaks=seq(0, 1, by =0.05),aes(x = PCTFLOAN, y=(..count..)), 
                       alpha=0.9,fill ="blue",position="dodge") +
  labs(title="Overlaying Histogram of PCTPELL and PCTFLOAN ") +
    theme(plot.title = element_text(hjust = 0.5)) + 
  scale_colour_manual("", 
                      breaks = c("PCTPELL", "PCTFLOAN"),
                      values = c("PCTPELL"="red", "PCTFLOAN"="blue")) 


```

aid_compare -> data_clean[c(5:6)]
class(aid_compare)
